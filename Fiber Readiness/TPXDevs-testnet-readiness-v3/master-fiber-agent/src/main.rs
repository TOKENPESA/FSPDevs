...
type PeerRegistry = Arc<RwLock<HashMap<u16, (mpsc::Sender<AxumMessage>, u64)>>>;
...
async fn background_processor_worker(
    ...
    while let Some(telemetry) = rx.recv().await {
        ...
        if let Some((agent_tx, _)) = registry.get(&telemetry.agent) {
            if agent_tx.try_send(AxumMessage::Text(serialized)).is_err() {
                eprintln!(
                    "❌ [HEALING FAILURE] Failed to route instruction to Node FA-{}. Target dropped channel link.",
                    telemetry.agent
                );
            } else {
                let ui_event = format!(
            ...
            eprintln!(
                "⚠️ [HEALING VOID] Instruction dropped for Node FA-{}. No active WebSocket connection available.",
                telemetry.agent
            );
        }
    }
...
async fn ws_handler(
    ...
    if let Some(origin) = headers.get(header::ORIGIN) {
        ...
        let is_valid = origin_str == "http://127.0.0.1:8088"
            || origin_str == "http://localhost:8088"
            || origin_str == "http://127.0.0.1:8787"
            || origin_str == "http://localhost:8787";
        if !is_valid {
            eprintln!(
                "⚠️ [SECURITY VIOLATION] Rejected WebSocket Upgrade for Domain: {origin_str}"
            );
...
async fn handle_socket(socket: WebSocket, agent_id: u16, peers: PeerRegistry) {
    ...
    let current_conn_id = NEXT_CONN_ID.fetch_add(1, AtomicOrdering::Relaxed);
    peers
        .write()
        .await
            ...
            result = rx.recv() => {
                match result {
                    ...
                    Err(broadcast::error::RecvError::Lagged(skipped_frames)) => {
                        eprintln!(
                            "⚠️ [MONITOR CHANNEL] Client lagging behind event stream. Skipped {skipped_frames} frames."
                        );
                        let catchup_alert = format!(
                            "{{\"event\":\"SYS_LAG\",\"skipped\":{skipped_frames}}}"
                        );
                        if ws_tx.send(AxumMessage::Text(catchup_alert)).await.is_err() {
                            break;
                        }
                    }
